		...
		<div class="row">
			...
			<div class="col-md-6">
				<table class="table">
					...
					<tbody style="font-size: 13px">
						<c:forEach items="${questionList }" var="board">
							<c:if test="${board.questionNoticeYn eq 'Y'}">
								<tr style="background-color: rgb(233,233,233);">
									<td style="width: 5%; font-size: 12px" class="">${board.questionBoardNo }</td>
									<td style="width: 25%; font-size: 14px; font-weight: bold;" class=""><a href="questionview?boNo=${board.questionBoardNo }">${board.questionBoardTitle }</a></td>
									<td style="width: 10%" class="">${board.questionBoardWriter }</td>
									<td style="width: 10%" class="">${board.questionBoardRegDate }</td>
									<td style="width: 10%" class="">${board.questionBoardHit }</td>
								</tr>
							</c:if>
							<c:if test="${board.questionNoticeYn ne 'Y'}">
								<tr>
									<td style="width: 10%; font-size: 12px" class="">${board.questionBoardNo }</td>
									<td style="width: 20%; font-size: 14px;" class=""><a href="questionview?boNo=${board.questionBoardNo }">${board.questionBoardTitle }</a></td>
									<td style="width: 10%" class="">${board.questionBoardWriter }</td>
									<td style="width: 10%" class="">${board.questionBoardRegDate }</td>
									<td style="width: 10%" class="">${board.questionBoardHit }</td>
								</tr>
				...
				<div class="d-flex flex-row-reverse bd-highlight"
					...
					<a href="questionform" class="btn btn-secondary"><i class="bi bi-pencil">&nbsp;</i>글쓰기</a>
				</div>
				...
				<!-- 페이지 -->
				<div
					style="background-color: rgba(220, 220, 220, 0.3); padding-top: 10px;">
					...
					<nav aria-label="Page navigation example"
						...
						<ul class="pagination1">
							<li class="page-item"><a class="page-link"
								href="questionboard?curPage=1" data-page="1"><span
									aria-hidden="true">&laquo;</span></a></li>
							<c:if test="${searchVO.firstPage ne 1 }">
								<li class="page-item"><a class="page-link"
									href="questionboard?curPage=${searchVO.firstPage - 1}"
							...
							<c:forEach begin="${searchVO.firstPage }"
								...
								<c:if test="${searchVO.curPage ne i }">
									<li class="pagination1"><a class="pagination1"
										href="questionboard?curPage=${i }" data-page="${i }">${i }</a></li>
								</c:if>
								<c:if test="${searchVO.curPage eq i }">
									<li class="pagination1 active" aria-current="page"><a
										class="pagination1 active" href="#">${i }</a></li>
								</c:if>
							</c:forEach>
							<c:if test="${searchVO.lastPage ne searchVO.totalPageCount }">
								<li class="page-item"><a class="page-link"
									href="questionboard?curPage=${searchVO.lastPage + 1}"
									data-page="${searchVO.lastPage + 1}" aria-label="Next"> <span
										aria-hidden="true">&gt;</span>
								</a></li>
							</c:if>
							<li class="page-item"><a class="page-link"
								href="questionboard?curPage=${searchVO.totalPageCount}"
			...
			<div class="panel-body">
				<form name="search" action="questionboard" method="post" class="form-horizontal">
