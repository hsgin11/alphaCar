...
public class ProdPagingVO implements Serializable {
	//입력받는 데이터
	private int curPage=1;           // 현재 페이지 번호
	private int rowSizePerPage=60;   // 한 페이지당 레코드 수      기본10
	


	private int rowSizePerPage2=10;   // 판매자 페이지당 레코드 수     
	...
	//	this.totalPageCount = totalRowCount % pageSize == 0 ? totalRowCount / 10 : (totalRowCount / 10) + 1;
		this.totalPageCount = (totalRowCount-1) / rowSizePerPage + 1;
	...
	//	index = 10
		this.totalPageCount = (totalRowCount-1) / rowSizePerPage2 + 1;
		firstRow = (curPage-1) * rowSizePerPage2 + 1;
		lastRow = firstRow + rowSizePerPage2 - 1;		
		if(lastRow > totalRowCount) lastRow = totalRowCount;
